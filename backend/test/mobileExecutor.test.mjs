...
test('mobileRunScenario performs configured swipes and like logging through adb', async () => {
  ...
  try {
    ...
    assert.equal((commandLog.match(/shell input tap 1332 1438/g) ?? []).length, 2)
    ...
    assert.match(emitted[5].details, /iteration=1 x=1332 y=1438/)
...
test('mobileRunScenario skips like when random chance does not hit', async () => {
  ...
  try {
    ...
    assert.equal((commandLog.match(/shell input tap 1332 1438/g) ?? []).length, 0)
  } finally {
    _resetMobileExecutorSessionForTests()
    fs.rmSync(tempDir, { recursive: true, force: true })
    fs.rmSync(commandLogFile, { force: true })
...
test('mobileRunScenario can reuse already opened TikTok app without reopening it', async () => {
  ...
  try {
    ...
    assert.equal((commandLog.match(/shell input swipe 720 1900 720 600 500/g) ?? []).length, 1)
    assert.equal((commandLog.match(/shell input tap 1332 1438/g) ?? []).length, 1)
    assert.deepEqual(
      emitted.map((entry) => entry.action),
