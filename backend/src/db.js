...
try {
  db.prepare(
    `UPDATE accounts SET mobile_device_id = '', mobile_vm_index = '' WHERE LOWER(TRIM(COALESCE(account_type, ''))) = 'mobile'`,
  ).run()
} catch (e) {
  console.error('[db] clear persisted mobile adb fields', e)
}
